...
export const policyVersionInput = z.object({
  ...
  if (value.effectiveTo && value.effectiveTo < value.effectiveFrom)
    context.addIssue({ code: "custom", path: ["effectiveTo"], message: "適用終了日は開始日以降です" });
});

export const goalPolicyLinkInput = z.object({
  policyItemId: id,
  relevanceNote: z.string().trim().max(2000).default(""),
}).strict();

export const reviewInput = z.object({
  targetType: z.enum(["GOAL_VERSION", "PROGRESS_ENTRY", "REFLECTION", "ONE_ON_ONE_ENTRY"]),
  targetId: id,
  unitId: id,
...
}).strict().refine((v) => v.periodEnd >= v.periodStart, {
  path: ["periodEnd"], message: "期間終了日は開始日以降です",
});

export const businessDayInput = z.object({
  targetMonth: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/),
  calendarVersion: z.string().trim().min(1).max(50),
  holidays: z.array(z.string().date()).max(100),
}).strict();

export const responseWindowInput = z.object({
