...
export class ExecutiveRepository {
  ...
  async overview(principal: Principal) {
    if (!principal.globalUnitRead)
      throw new MemberError("RESOURCE_NOT_FOUND", 404, "executive_overview_not_visible");
    ...
    return {
      ...
      prohibitedUses: ["人の順位付け", "離職予測", "意欲・心理状態の推定", "人事評価・給与の確定"],
    };
  }
  ...
  async listPolicies() {
    ...
    return {
      ...
      items: items.results,
      managementNotice: "Management項目はDRAFTであり、正式評価・通常AI Context・目標linkには使用しません",
    };
  }
  ...
  async createPolicyDocument(
    ...
    await this.db.batch([
      ...
      this.audit("POLICY_DOCUMENT_CREATED", principal, "policy_document", id, requestId, now),
    ]);
  ...
  async addPolicyVersion(
    ...
    const statements = [
      ...
      this.audit("POLICY_VERSION_REGISTERED", principal, "policy_version", versionId, requestId, now),
    ];
    try {
  ...
  async linkGoalPolicy(
    ...
    const predicate = globalWrite ? "" : " AND g.unit_id IN (" + placeholders + ")";
    const goal = await this.db
      .prepare(
        "SELECT gv.id FROM goal_versions gv JOIN goals g ON g.id=gv.goal_id " +
          "WHERE gv.id=? AND g.current_version_id=gv.id" + predicate,
      )
    ...
    try {
      await this.db.batch([
        this.db
          ...
          .bind(id, goalVersionId, policyItemId, relevanceNote, principal.actorId, now),
        this.audit("GOAL_POLICY_LINKED", principal, "goal_version", goalVersionId, requestId, now),
      ]);
    } catch {
      throw new MemberError("INVALID_STATE_TRANSITION", 422, "policy_link_not_eligible");
    }
  ...
  async listReviews(principal: Principal) {
    ...
    const predicate = scope.global
      ? ""
      : "WHERE r.unit_id IN (" + scope.unitIds.map(() => "?").join(",") + ")";
    const rows = await this.db
      .prepare(
        "SELECT r.*,u.code unit_code,u.name unit_name FROM review_requests r JOIN units u ON u.id=r.unit_id " +
          predicate + " ORDER BY CASE r.status WHEN 'RETURNED' THEN 0 WHEN 'UL_RESPONDED' THEN 1 WHEN 'UNCONFIRMED' THEN 2 ELSE 3 END,r.updated_at DESC",
      )
      .bind(...scope.unitIds)
      .all<Row>();
    const result = [];
  ...
  async createReview(
    ...
    try {
      ...
      throw new MemberError("RESOURCE_NOT_FOUND", 404, "review_target_not_visible");
    }
  ...
  async reviewUnit(reviewId: string) {
    const row = await this.db
      .prepare("SELECT unit_id,status,version FROM review_requests WHERE id=?")
      .bind(reviewId)
      .first<{ unit_id: string; status: string; version: number }>();
    if (!row) throw new MemberError("RESOURCE_NOT_FOUND", 404, "review_not_visible");
    return row;
  }
  ...
  async addReviewComment(
    ...
    if (!allowed[current.status]?.includes(input.disposition))
      throw new MemberError("INVALID_STATE_TRANSITION", 422, "invalid_review_transition");
    const now = new Date().toISOString();
    ...
    const result = await this.db.batch([
      ...
      this.audit("REVIEW_ACTION_RECORDED", principal, "review", reviewId, requestId, now, true),
    ]);
  ...
  async calculateTurnoverForUnit(
    ...
    const result = await this.db.batch([
      ...
      this.audit("TURNOVER_CALCULATED", principal, "unit", unitId, requestId, now, true),
    ]);
    const saved = await this.db
      ...
      .first<Row>();
    return { ...saved, created: Boolean(result[0]?.meta.changes), disclaimer: calculation.disclaimer };
  }

  calculateBusinessDay(targetMonth: string, calendarVersion: string, holidays: string[]) {
    return {
      targetMonth,
      dueDate: secondBusinessDayOfFollowingMonth(targetMonth, new Set(holidays)),
      calendarVersion,
  ...
  calculateResponseWindow(contactAt: string, responseAt: string | null, referenceAt: string) {
