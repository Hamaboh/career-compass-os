...
export class AdminRepository {
  ...
  async executeRetention(
    ...
    await this.db.batch([
      ...
      this.db
        .prepare(
          `UPDATE retention_actions
           SET subject_id='retired:'||lower(hex(randomblob(16))),
               preview_json='{"anonymized":true,"retained":"anonymous cohort statistics only"}',
               preview_hash=lower(hex(randomblob(32))),
               status='EXECUTED',result_json=?,updated_at=?
           WHERE id=? AND status='EXECUTING' AND subject_type='MEMBER'`,
        )
        .bind(
          JSON.stringify({
            anonymized: true,
  ...
  async createBackupExport(
    ...
    const existing = await this.db
      .prepare(
        "SELECT id,environment,status,object_key,source_timestamp FROM backup_exports WHERE created_by=? AND idempotency_key=?",
      )
    ...
    if (
      ...
      (existing.environment !== input.environment ||
        existing.source_timestamp !== input.sourceTimestamp)
    )
      throw new MemberError(
        "VERSION_CONFLICT",
        409,
    ...
    if (existing?.status === "READY") return this.dashboard();
    ...
    const objectKey =
      existing?.object_key ??
      `backups/${input.environment.toLowerCase()}/${now.slice(0, 10)}/${id}.json`;
    // Build the complete artifact before claiming a failed row so a read/export
    // failure cannot strand the idempotency key in PENDING.
    ...
    if (existing) {
      const reclaimed = await this.db
        .prepare(
          "UPDATE backup_exports SET status='PENDING',manifest_checksum=NULL WHERE id=? AND status='FAILED'",
        )
        .bind(id)
        .run();
      if ((reclaimed.meta.changes ?? 0) !== 1)
        throw new MemberError("VERSION_CONFLICT", 409, "backup_retry_conflict");
    }
    try {
      if (existing) {
        // The failed row was atomically reclaimed above; reuse its immutable id/key.
      } else {
        await this.db
          .prepare(
