...
export async function POST(
  ...
  return withMemberRuntime(req, rt, async (_, p, rid) => {
    ...
    return success(
      await repo.addQuestion(p, id, await strictJson(req, questionInput), rid),
      rid,
      null,
      201,
    );
...
export async function PATCH(
  ...
  return withMemberRuntime(req, rt, async (_, p, rid) => {
    assertMutationRequest(req);
    const repo = await writeRepository(req, rt.db, p, rid, id),
      body = await strictJson(req, questionUpdateInput);
    return success(
      await repo.updateQuestion(p, id, body.questionId, body, rid),
      rid,
    );
  });
}
